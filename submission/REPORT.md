...
|---|---|---|---|
| Lê Kiên Cường | Logging & PII (correlation ID middleware, structured logging, PII scrubbing); Metrics & Alerting (error_rate_pct, SLO config, alert rules, runbook, dashboard contract); Tracing & Prompt Versioning (sub-span observe decorator, prompt v1/v2, label switch/rollback); Điều tra challenge CP3 | _(điền link commit)_ | Cách thiết kế alert symptom-based; kỹ thuật đối chiếu Metrics -> Traces -> Logs để xác định root cause; phát hiện vấn đề blocking event loop trong FastAPI async handler; quản lý biến môi trường (.env vs shell export) |
| Xuân Thế Độ | Security & Compliance (PII scrubbing patterns, redaction coverage toàn bộ log fields) | _(điền link commit)_ | _(điền)_ |
| Trần Công Đức | Logging & Middleware (Correlation ID middleware, structured logging với structlog, JSON schema log) | _(điền link commit)_ | _(điền)_ |
| Nguyễn Công Trí | QA & Incident Analyst (chạy pytest, validate scripts, hỗ trợ điều tra incident, viết báo cáo) | _(điền link commit)_ | _(điền)_ |